...
float trim_tab_angle = 0;
volatile float angle;


#define mechanical_offset 0  //mechanical offset to be determined
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        if (rxHeader.Identifier == 0x002 ) {



        		printf("Servo command was received\r\n");

        		//every two hex digit is one byte of data !

                recieved_msg =  ((uint32_t)rxData[0])        |
                                              ((uint32_t)rxData[1] << 8)   |
                                              ((uint32_t)rxData[2] << 16)  |
                                              ((uint32_t)rxData[3] << 24);

                printf("the hex value recieved was 0x%X\r\n" , recieved_msg);
